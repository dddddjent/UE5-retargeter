...
// Define log category for this commandlet
DEFINE_LOG_CATEGORY_STATIC(RetargeterCommandlet, Log, All);
...
static int32 ParseArgs(const FString& Params, FString& OutInputFbx, FString& OutTargetFbx, FString& OutOutputPath, bool& bOutPersist)
{
    ...
    if (!FParse::Value(*Params, TEXT("input="), OutInputFbx) || OutInputFbx.IsEmpty()) {
        UE_LOG(RetargeterCommandlet, Error, TEXT("Missing required argument: -input=<path to input fbx>"));
        return 1;
    }
    if (!FParse::Value(*Params, TEXT("target="), OutTargetFbx) || OutTargetFbx.IsEmpty()) {
        UE_LOG(RetargeterCommandlet, Error, TEXT("Missing required argument: -target=<path to target fbx>"));
        return 2;
    }
    if (!FParse::Value(*Params, TEXT("output="), OutOutputPath) || OutOutputPath.IsEmpty()) {
        UE_LOG(RetargeterCommandlet, Error, TEXT("Missing required argument: -output=<output path>"));
        return 3;
    }
    ...
    UE_LOG(RetargeterCommandlet, Display, TEXT("Input: %s"), *OutInputFbx);
    UE_LOG(RetargeterCommandlet, Display, TEXT("Target: %s"), *OutTargetFbx);
    UE_LOG(RetargeterCommandlet, Display, TEXT("Output: %s"), *OutOutputPath);

    // Basic existence checks for inputs
    if (!FPaths::FileExists(OutInputFbx)) {
        UE_LOG(RetargeterCommandlet, Error, TEXT("Input file not found: %s"), *OutInputFbx);
        return 4;
    }
    if (!FPaths::FileExists(OutTargetFbx)) {
        UE_LOG(RetargeterCommandlet, Error, TEXT("Target file not found: %s"), *OutTargetFbx);
        return 5;
    }
    ...
    if (!OutputDir.IsEmpty() && !FPaths::DirectoryExists(OutputDir)) {
        if (!IFileManager::Get().MakeDirectory(*OutputDir, true)) {
            UE_LOG(RetargeterCommandlet, Error, TEXT("Failed to create output directory: %s"), *OutputDir);
            return 6;
        }
    }
...
{
    UE_LOG(RetargeterCommandlet, Display, TEXT("---Retargeting a pair of animations---"));
    ...
    UE_LOG(RetargeterCommandlet, Display, TEXT("Arguments validated. Proceeding with retargeting..."));
